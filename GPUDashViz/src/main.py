...
def serve_scatter(start_date, end_date, hue):
    if isinstance(hue, bool):
        ...
        # Use Plotly's scatter_matrix (similar to Seaborn's pairplot)
        fig = px.scatter_matrix(
            df_subset,
            dimensions=[col for col in df_subset.columns if col != 'TYPE'],
            color="TYPE" if hue else None,
            title="Pairs Plot"
        )
        
        # Customize layout to match your preferred style
        fig.update_layout(
            autosize=False,
            width=800,
            height=800,
